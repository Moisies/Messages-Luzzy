...
class MessagingUtils(val context: Context) {
    ...
    private fun getSmscForSubscription(subId: Int): String? {
        try {
            ...
            if (subscriptionInfo != null) {
                ...
                val carrierName = subscriptionInfo.carrierName?.toString()?.lowercase()

                android.util.Log.e("LUZZY_DEBUG", "=== getSmscForSubscription: subId=$subId, mcc=$mcc, mnc=$mnc, carrier=$carrierName")

                if (mcc == "732") {
                    return when (mnc) {
                        "103", "111" -> {

                            android.util.Log.e("LUZZY_DEBUG", "=== Using TIGO SMSC: +573009700099")
                            "+573009700099"
                        }
                        "123" -> "+573012000732"
                        "101" -> "+573011000101"
            ...
            android.util.Log.e("LUZZY_DEBUG", "=== Error getting SMSC: ${e.message}", e)
        }
    ...
    fun sendSmsMessage(
        ...
        for (address in addresses) {
            ...
            try {
                val serviceCenter = getSmscForSubscription(subId)
                context.smsSender.sendMessage(
                    subId = subId, destination = address, body = text, serviceCenter = serviceCenter,
